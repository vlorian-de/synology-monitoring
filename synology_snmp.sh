...
influxdb_token="1234567812345678123456781234567812345678123456781234567812345678abcdabcdabcdabcd12345678=="
influxdb_org="your-org"
influxdb_bucket_name="nas" #influxdb bucket name, must be created first
...
"
	...
	if [[ -z $influxdb_user ]]; then
		...
		if [[ -z $influxdb_token ]]; then
			...
			curl -i -XPOST "$http_method://$influxdb_host:$influxdb_port/api/v2/write?org=$influxdb_org&bucket=$influxdb_bucket_name&precision=s" --data-binary "$post_url" --header "Authorization: Token $influxdb_token" --header "Content-Type: text/plain; charset=utf-8" --header "Accept: application/json"
		fi
	fi
	echo "$post_url"
